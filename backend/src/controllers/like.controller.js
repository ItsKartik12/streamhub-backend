...
const toggleVideoLike = asyncHandler(async (req, res) => {
    ...
    if (existingLike) {
        ...
        return res.status(200).json(new ApiResponse(200, { liked: false, likeCount }, "Video unliked successfully"))
    }
    await Like.create({ video: videoId, likedBy: req.user._id })
    const likeCount = await Like.countDocuments({ video: videoId })
    return res.status(201).json(new ApiResponse(201, { liked: true, likeCount }, "Video liked successfully"))
})
...
const toggleCommentLike = asyncHandler(async (req, res) => {
    ...
    return res.status(201).json(new ApiResponse(201, { liked: true }, "Comment liked successfully"))
})
...
const toggleTweetLike = asyncHandler(async (req, res) => {
    ...
    return res.status(201).json(new ApiResponse(201, { liked: true }, "Tweet liked successfully"))
}
)
...
const getLikedVideos = asyncHandler(async (req, res) => {
    ...
    return res.status(200).json(new ApiResponse(200, { docs: videos, totalDocs: videos.length }, "Liked videos fetched successfully"))
})
